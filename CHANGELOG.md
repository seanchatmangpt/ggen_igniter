...
- **`mix ggen_igniter.hand_authored` — the hand-authored-source admission
  task** (`feat/hand-authored-admit-task`): automates the whole lawful
  admission the `beam4pm-process-model-pack` (v0.1.10+) expresses as
  `bpm:HandAuthoredSource` / `bpm:AuthorshipKind` facts and enforces with
  gates 050/060/070 plus the consumer's rendered
  `scripts/gate_authorship_check.sh`. Three verbs:

  - `admit` validates every capability-object law BEFORE writing anything —
    file exists on disk, no `GENERATED by ggen` marker (REFUSED_MARKED_FILE),
    path under a manufactured root (REFUSED_MISSCOPED), acceptance command
    present, expiry well-formed, kind in the pack's CLOSED vocabulary
    (REFUSED_UNKNOWN_KIND, the anti-join law of gate 060 evaluated
    pre-write), debt kinds carry `--sunset-plan` + `--expires`
    (REFUSED_MISSING_SUNSET_PLAN / REFUSED_MISSING_EXPIRY — a path-only debt
    admission is a standing grant), no duplicate `bpm:sourcePath`
    (REFUSED_DUPLICATE_SOURCE_PATH), and the kind's ceiling not exceeded
    (REFUSED_CEILING_EXCEEDED, with the pack-edit remedy named) — then
    computes the real sha256 over the real bytes, appends the individual to
    the consumer's ontology.ttl (directly after the last existing
    `bpm:HandAuthoredSource` individual, or end-of-file when none exists),
    and triggers the pack render: the three hand-authored projections are
    Tera templates on the Rust `ggen sync run` leg, whose write policy
    (FM-WRITE-005) refuses to overwrite existing targets from non-force
    templates, so the task performs the same scaffold-`force: true` → sync →
    restore-verbatim procedure the v26.9.18 integration used and documented,
    restoring the template bytes and refusing loudly on a failed restore
    (RENDER_TEMPLATE_RESTORE_FAILED). `--no-render` skips the render (with
    the stale-manifest consequence printed); missing `ggen` binary is
    `BLOCKED_RENDER_NO_GGEN` (exit 3) with the exact two-step manual
    commands; `--dry-run` runs every validation for real and writes nothing.
  - `list` prints the admitted set per kind (count/ceiling, AT CEILING
    flagged) plus one row per admission, then REALLY runs the pack's own
    `gates/0{50,60,70}_*.rq` through the oxigraph engine over the merged
    consumer+pack graph (exit 1 on any gate row).
  - `check` runs the consumer's rendered shell gate as a real subprocess,
    streaming output, exiting with the gate's own code.

  Typed refusals exit 1, invalid invocation exits 2, blocked render exits 3.
  Proven on the real beam4pm consumer at parity/integration: the v26.9.18
  parity wave's 17 hand-in-lockstep admissions (11 from the synthesis commit
  + the 6 TTL corpus files the post-render gate surfaced) were reverted to
  the pre-wave ledger (47 admitted / 40 debt) and re-admitted one by one
  THROUGH the task, ending at the same gate state the integration branch
  held — GATE AUTHORSHIP PASS, 64 admitted / 57 counted as debt, 0 findings.
