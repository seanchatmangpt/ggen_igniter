...
| `--verify-base-sha` | boolean | `false` | Opt-in git ground truth for semantic WorkOrders: every rendered row carrying a `base_sha` column must name a commit reachable from HEAD in `--verify-cwd` (`git cat-file -e <sha>^{commit}` AND `git merge-base --is-ancestor <sha> HEAD`, both exit 0), else the typed refusal `REFUSED:SEMANTIC_JIRA_BASE_SHA_UNVERIFIED` raises BEFORE any write. Verification is scoped to jurisdiction (see `GgenIgniter.SemanticJira.GitGroundTruth`'s moduledoc): only rows whose `repository` matches the verifying work tree's OWN repository — the exact `owner/repo` parsed from its `remote.origin.url`, compared EXACTLY, never by substring — are verified; rows with no `repository` column, and work trees with no origin remote, are in jurisdiction. Under this flag an out-of-jurisdiction row is a per-run typed skip, printed as a `SKIPPED:SEMANTIC_JIRA_BASE_SHA_OUTSIDE_JURISDICTION` line and never silently dropped; that same row opted in per order via `requires_git_ground_truth` is instead a refusal (the graph demanded ground truth this work tree cannot establish). Never default-on (canonical WorkOrders may legitimately carry baseShas absent from the verifying checkout). Independently, a WorkOrder row with `requires_git_ground_truth` verifies that order's baseSha with no flag. |
