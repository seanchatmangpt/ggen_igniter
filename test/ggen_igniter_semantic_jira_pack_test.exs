defmodule GgenIgniter.SemanticJiraPackTest do
  ...
  describe "Vision projection template (V23-T6R: the adopted WO-03 template, executed)" do
    @tag timeout: @double_sync_timeout
    test "manufactures the vision projection for the dogfood set and replays byte-identically" do
      ...
      # Checkpoint ladder, next actions and projection surface are graph-derived.
      assert first_bytes =~ "## Checkpoint ladder"
      assert first_bytes =~ "**SHACL admission checkpoint**"
      assert first_bytes =~ "**Add a qualified SHACL court**"
      assert first_bytes =~ "**Jira-shaped Markdown projection**"
      ...
      assert File.read!(output_path) == first_bytes
      assert length(Receipt.read_all!(work_dir)) == 2
    end
    ...
    test "removing a required baseSha refuses the vision projection before any file is actuated" do
      ...
      assert Enum.all?(Receipt.read_all!(work_dir), &(&1["standing"] != "alive"))
    end
  end
